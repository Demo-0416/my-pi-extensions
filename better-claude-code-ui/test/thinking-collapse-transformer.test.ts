 ...
 * 展开态返回 `∴ Thinking…` 标题+正文。ctrl+shift+t / alt+t 切换,并借
...
test("ctrl+shift+t 展开出标题+正文,再按折叠回空;宿主重建通道被触发", async () => {
	const { pi, transform } = setup();
	await pi.emit("session_start", { reason: "startup" });
	const shortcut = pi.shortcuts.get("ctrl+shift+t") as { handler: (ctx: unknown) => Promise<void> };
	assert.ok(shortcut, "应注册 ctrl+shift+t");
	assert.ok(pi.shortcuts.get("alt+t"), "应注册 alt+t 降级键(非 Kitty 终端)");
